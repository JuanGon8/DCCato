...
$sql = "INSERT INTO reclutamiento_a SELECT * FROM reclutamiento_baja_a WHERE codigo = ?";
if ($stmt = $conn->prepare($sql)) {
    ...
    if ($stmt->execute()) {
        ...
        $updateSql = "UPDATE reclutamiento_a SET estatus_emp = 'R', fecha_reingreso = NOW() WHERE codigo = ?";
...
$sql = "DELETE FROM reclutamiento_baja_a WHERE codigo = ?";
if ($stmt = $conn->prepare($sql)) {
    $stmt->bind_param("i", $codigo); // "i" indica un entero (cambia si el campo no es un entero)
